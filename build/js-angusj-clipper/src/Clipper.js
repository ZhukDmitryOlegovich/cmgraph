import { clipTypeToNative, polyFillTypeToNative, polyTypeToNative } from "./native/nativeEnumConversion.js";
import { nativePathsToPaths, pathsToNativePaths } from "./native/PathsToNativePaths.js";
import { pathToNativePath } from "./native/PathToNativePath.js";
import { PolyTree } from "./PolyTree.js";
import { nativeFinalizationRegistry } from "./nativeFinalizationRegistry.js";
export class Clipper {
     ...
     * @param initOptions
     */
    constructor(_nativeLib, initOptions = {}) {
     ...
     */
    executeToPaths(clipType, subjFillType, clipFillType, cleanDistance) {
        const outNativePaths = new this._nativeLib.Paths();
        try {
            const success = this._clipper.executePathsWithFillTypes(clipTypeToNative(this._nativeLib, clipType), outNativePaths, polyFillTypeToNative(this._nativeLib, subjFillType), polyFillTypeToNative(this._nativeLib, clipFillType));
            if (!success) {
                return undefined;
            }
            else {
     ...
     */
    executeToPolyTee(clipType, subjFillType, clipFillType) {
        const outNativePolyTree = new this._nativeLib.PolyTree();
        try {
            const success = this._clipper.executePolyTreeWithFillTypes(clipTypeToNative(this._nativeLib, clipType), outNativePolyTree, polyFillTypeToNative(this._nativeLib, subjFillType), polyFillTypeToNative(this._nativeLib, clipFillType));
            if (!success) {
                return undefined;
            }
            else {
